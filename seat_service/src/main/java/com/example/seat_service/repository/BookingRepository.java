...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    // BookingRepository.java — add these
    @Query("SELECT COUNT(b) FROM Booking b WHERE b.bookedAt >= :from AND b.status = :status")
    Long countByBookedAtAfterAndStatus(@Param("from") Instant from, @Param("status") BookingStatus status);
